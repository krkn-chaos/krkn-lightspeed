...
"""
krknctl-compatible RAG pipeline using
FAISS + llama.cpp + multi-qa-mpnet-base-dot-v1
...
def load_llama31_krknctl_rag_pipeline(
    ...
    if os.path.exists(model_path):
        ...
        if gpu_config["backend"] == "vulkan":
            llama_params.update(
                {
                    "split_mode": 1,
                    # Vulkan-specific: layer split mode
                    "tensor_split": None,
                    # Vulkan-specific: tensor split configuration
                    "main_gpu": 0,
                    # Vulkan-specific: main GPU selection
                    "offload_kqv": True,
                    # Vulkan-specific: offload key-query-value to GPU
                }
            )
        ...
        logger.info(
            "🔄 CODE VERSION: 2024-10-31-v7-VULKAN-NATIVE"
        )  # Version marker
    ...
    def generate(state: State):
        ...
        with backend-optimized parameters"""
        if not llama_model:
            return {
...
Answer based ONLY on the context provided:"""  # NOQA
        ...
        try:
            # Backend-optimized parameters for
            ...
            if backend == "cuda":
                ...
                inference_params = {
                    ...
                    # Keep penalty
                    "repeat_penalty": 1.15,
                    # Vulkan-specific: fixed seed for reproducibility
                    "seed": 42,
                    # Vulkan-specific: tail free sampling disabled
                    "tfs_z": 1.0,
                    # Vulkan-specific: typical sampling disabled
                    "typical_p": 1.0,
                    "echo": False,
                }
            ...
            response = llama_model(prompt, **inference_params)
            ...
            logger.info(
                f"Prompt tokens: {usage.get('prompt_tokens', 'unknown')}"
            )
            logger.info(
                f"Completion tokens: "
            ...
            logger.info(
                f"Total tokens: " f"{usage.get('total_tokens', 'unknown')}"
            )
            if "timings" in response:
                timings = response["timings"]
                logger.info(
                    f"Prompt eval time: "
                    f"{timings.get('prompt_eval_time', 'unknown')}ms"
                )
                logger.info(
                    f"Eval time: {timings.get('eval_time', 'unknown')}ms"
                )
                logger.info(
                    f"Tokens per second: "
                    f"{timings.get('tokens_per_second', 'unknown')}"
                )
            logger.info("=========================")
